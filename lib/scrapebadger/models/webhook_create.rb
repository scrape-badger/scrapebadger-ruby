  ...
  class WebhookCreate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ScrapeBadger::WebhookCreate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ScrapeBadger::WebhookCreate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@secret.nil? && @secret.to_s.length > 128
        invalid_properties.push('invalid value for "secret", the character length must be smaller than or equal to 128.')
      end
    ...
    def valid?
      ...
      return false if !@secret.nil? && @secret.to_s.length > 128
      true
    end
    ...
    def secret=(secret)
      if !secret.nil? && secret.to_s.length > 128
        fail ArgumentError, 'invalid value for "secret", the character length must be smaller than or equal to 128.'
      end
