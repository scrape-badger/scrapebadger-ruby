  ...
  class WebhookListItem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ScrapeBadger::WebhookListItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ScrapeBadger::WebhookListItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @secret_set.nil?
        invalid_properties.push('invalid value for "secret_set", secret_set cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @secret_set.nil?
    ...
    def hash
      [id, monitor_id, monitor_name, url, secret_set, created_at].hash
    end
